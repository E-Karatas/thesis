...
# For example, if you want to have 75% of the data in the training set and 25% in the testing set
train_size = 0.75
test_size = 0.25
...
for label in class_distribution.index:
    ...
    # Randomly select indices for training and testing sets while maintaining the class distribution
    train_indices.extend(np.random.choice(class_indices, train_class_counts[label], replace=False))
    test_indices.extend(np.random.choice(class_indices, test_class_counts[label], replace=False))

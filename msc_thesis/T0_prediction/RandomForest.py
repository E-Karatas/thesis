...
# %%
rf = RandomForestRegressor(n_estimators=60, random_state=1)
...
y_pred_cv = cross_val_predict(rf, X, y, cv=crossvalidation)
...
y_pred_bio = rf.predict(X_bio)

# Calculate R2 and RMSE for the new predictions
r2_bio = rf.score(X_bio, y_bio)
...
plt.xlabel('True T0 [K]')
plt.ylabel('Predicted T0 [K]')
plt.title('Random Forest - True vs Predicted T0 (Bio Dataset)')
plt.legend()
plt.grid(True)
...
y_pred_bio_cv = cross_val_predict(rf_cv, X_bio, y_bio, cv=crossvalidation)

# Calculate R2 and RMSE for the new predictions
r2_bio_cv = rf_cv.score(X_bio, y_bio)
...
plt.xlabel('True T0 [K]')
plt.ylabel('Predicted T0 [K]')
plt.title('Random Forest - True vs Predicted T0 (Bio Dataset - CV Model)')
plt.legend()
plt.grid(True)
